...
const getLevelColor = (level: string) => {
    ...
    if (lvl === 'expert') return 'text-cyan-400 border-cyan-500';
    if (lvl === 'intermediate') return 'text-blue-400 border-blue-500';
    return 'text-gray-400 border-gray-500';
};

export default async function KeahlianPage() {
  const { biodata, keahlian } = await getData();
    ...
    <main className="min-h-screen bg-[#111111] text-gray-300 font-sans flex justify-center items-center p-4 md:p-10">
      <div className="flex w-full max-w-5xl bg-[#1a1a1a] shadow-2xl overflow-hidden rounded-lg aspect-[15/9]">
        ...
        <aside className="hidden md:flex w-20 bg-[#1a1a1a] flex-col items-center py-10 space-y-8 border-r border-gray-800 z-50">
          ...
          <Link href="/keahlian"><FaBolt className="text-cyan-400 cursor-pointer transition text-xl" /></Link> {/* Aktif */}
        ...
        <div className="flex flex-col md:flex-row w-full">
          ...
          <section className="hidden md:block w-full md:w-[35%] relative bg-[#121212] border-r border-gray-800">
             ...
             <div className="relative z-20 h-full flex flex-col justify-end p-10 bg-gradient-to-t from-[#1a1a1a] via-transparent to-transparent">
                ...
                <p className="text-cyan-400 text-sm tracking-widest uppercase mt-1">Skills & Expertise</p>
                {/* Sosmed dihapus */}
             </div>
          </section>
          ...
          <section className="w-full md:w-[65%] p-8 md:p-16 overflow-y-auto h-full max-h-[800px]">
            <div className="mb-10">
              <h2 className="text-4xl font-bold text-white mb-4">Keahlian</h2>
              <div className="h-1 w-20 bg-cyan-500"></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {keahlian.length > 0 ? (
                keahlian.map((item) => (
                  <div key={item.id} className={`bg-[#222] p-4 rounded border border-gray-800 hover:border-cyan-500 transition group`}>
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="text-lg font-bold text-white group-hover:text-cyan-400">{item.nama_skill}</h3>
                        <span className={`text-xs px-2 py-1 rounded border ${getLevelColor(item.level)} bg-black/30`}>{item.level}</span>
                    </div>
                    <p className="text-gray-500 text-xs uppercase tracking-wide bg-[#1a1a1a] inline-block px-2 py-1 rounded">{item.kategori}</p>
                  </div>
                ))
              ) : (<div className="text-gray-500">Data keahlian belum diisi.</div>)}
            </div>
          </section>
        </div>
      </div>
